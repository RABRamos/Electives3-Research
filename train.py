...
x = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[
                   None, img_size, img_size, num_channels], name='x')

# labels
y_true = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[None, num_classes], name='y_true')
...
def create_weights(shape):
    return tf.compat.v1.Variable(tf.compat.v1.truncated_normal(shape, stddev=0.05))


def create_biases(size):
    return tf.compat.v1.Variable(tf.compat.v1.constant(0.05, shape=[size]))
...
def create_flatten_layer(layer):
    ...
    layer = tf.compat.v1.reshape(layer, [-1, num_features])
